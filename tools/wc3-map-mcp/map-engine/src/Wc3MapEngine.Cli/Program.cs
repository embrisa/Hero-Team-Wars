    ...
    private static EngineResponse Handle(EngineRequest request)
    {
        ...
        try
        {
            var result = request.Operation switch
            {
                ...
                "object_field_lookup" => ObjectFieldCatalog.Lookup(request.Payload),
                "object_field_search" => ObjectFieldCatalog.Search(request.Payload),
                "object_field_annotations" => new JsonObject { ["object_fields"] = ObjectFieldCatalog.Annotate(request.Payload["definitions"]!.AsArray()) },
