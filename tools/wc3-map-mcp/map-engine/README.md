...
The current writer supports proven map metadata (`title`, `suggested_players`), complete typed `war3map.w3r` region records (including create/update/rename/delete/reorder, WTS token preservation, stable native identities, and reference-aware MCP operations), and hash-checked replacement of the existing JASS `war3map.j` entry point when MCP-owned script policy is enabled. Unknown members are preserved through rebuilds; GUI trigger, object, placement, and terrain mutation remain blocked until separate round-trip evidence exists.
