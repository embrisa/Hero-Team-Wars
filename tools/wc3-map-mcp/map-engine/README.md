...
The current writer supports proven map metadata (`title`, `suggested_players`), region bounds, and hash-checked replacement of the existing JASS `war3map.j` entry point when MCP-owned script policy is enabled. Unknown members are preserved through rebuilds; GUI trigger, object, placement, terrain, and generic archive mutation remain blocked until separate round-trip evidence exists.
