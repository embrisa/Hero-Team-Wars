...
import { ObjectFieldService } from "./services/object-field-service.js";
...
export function createServer(config: Wc3Config = loadConfig()): McpServer {
  ...
  const objectFields = new ObjectFieldService(worker);
  const server = new McpServer({ name: "wc3-map-mcp", version: "0.1.0" }, { instructions: "Inspect status and source hash first. The original map is immutable; mutations require a hash-tied transaction. Review the transaction diff and validate before building. For unfamiliar object fields, use wc3_object_field_search and wc3_object_field_lookup; readable field names resolve through the pinned catalog. Never guess field IDs, types, enum values or native scopes. For JASS, never invent names/signatures: use jass_search when unsure and jass_lookup for exact declarations/docs; jassdoc is canonical. Generated JASS is validated before map writes—correct failures and retry. Unsupported values stay unknown; destructive actions are limited to one confirmed MCP-owned transaction directory." });
  registerTools(server, { config, projects, inspections, transactions, builds, launches, gameplay, jass, objectFields });
  return server;
}
