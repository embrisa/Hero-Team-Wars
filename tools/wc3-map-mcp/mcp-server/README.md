...
The checked-in example configuration is `read_only` and allow-lists exactly six tools: `wc3_project_status`, `wc3_inspect_map`, `wc3_list_archive_files`, `wc3_get_component`, `wc3_validate_map`, and `wc3_compare_maps`. In that mode mutation, build, launch, evidence, promotion, and discard tools are not registered. A separately approved project configuration with `write_policy: "writes"` can enable the Phase 2 transaction tools; an empty allow-list enables the implementation's supported surface, while explicit allow-lists remain enforced per project.
