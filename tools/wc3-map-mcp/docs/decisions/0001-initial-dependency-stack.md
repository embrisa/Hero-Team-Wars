...
The worker boundary is deliberately small and versioned. It adds process startup cost, but gives the MCP layer a clear failure boundary and ensures diagnostic text cannot contaminate protocol stdout. Unsupported script, object, trigger, and opaque members remain read-only until a round-trip proof exists.
