...
assert(['stage', 'build', 'rebuild'].includes(mode) && evidenceDirectory, 'Usage: build-v27-icon-hud.mjs stage|build|rebuild <project-relative-evidence-directory>');
...
const baselineHash = '16C577CB12F73C30B86C5FD8F870106F0EF5CDE337795A50CE8D26B63B8C617C';
const baseline = resolve(root, 'builds/mcp/hero-team-wars/5e3fc540-35d8-49e6-93c1-fea98b42c5dc/HeroTeamWars_v26-preparation-and-sends_5e3fc540-35d8-49e6-93c1-fea98b42c5dc.w3m');
...
function preservedGlobals(source) {
  ...
  return block[1].split(/\r?\n/).map(line => line.trim()).filter(line =>
    line && !line.startsWith('//') && !/\bHTW_(?:InformationBoard\b|Hud\w*\b)/.test(line)).sort();
}
...
if (mode === 'stage') {
  ...
  project.root = root;
  copyFileSync(baseline, join(evidence, 'baseline-v26.w3m'), constants.COPYFILE_EXCL);
  project.source_maps = [rel(join(evidence, 'baseline-v26.w3m'))];
...
try {
  await request('initialize', { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'v27-icon-hud', version: '1' } });
  ...
  if (mode === 'stage') {
    ...
    const tx = await call('wc3_begin_transaction', { project_id: projectId, map: sourceMap, expected_source_hash: baselineHash, label: 'v27 compact icon HUD' });
    ...
    assert.deepEqual(preservedGlobals(script.source), preservedGlobals(oldScript.source), 'Changed non-HUD globals');
    for (const name of newFunctions.keys()) assert(oldFunctions.has(name) || name.startsWith('HTW_Information_'), `Unexpected new gameplay function: ${name}`);
    const changed = [...oldFunctions].filter(([name, body]) => newFunctions.get(name) !== body).map(([name]) => name);
    for (const [name, body] of oldFunctions) {
      if (!name.startsWith('HTW_Information_')) assert.equal(newFunctions.get(name), body, `Unintended baseline change: ${name}`);
    }
    const operations = [];
    operations.push({ operation_id: randomUUID(), type: 'set_script_source', target: { archive_path: 'war3map.j' }, expected: script.source_sha256 ?? script.sha256, value: { language: 'jass', source: script.source, source_strategy: 'composed' }, rationale: 'Install reviewed compact icon HUD with hover tooltips; preserve all v26 gameplay functions and objects.' });
    ...
    const result = await call('wc3_build_map', { project_id: projectId, transaction_id: state.transaction_id, revision: 1, expected_source_hash: baselineHash, profile: 'debug', label: 'v27-icon-hud' });
    ...
    for (const object of before.object_data) assert.deepEqual(after.object_data.find(o => o.id === object.id), object, `Changed v26 object ${object.id}`);
