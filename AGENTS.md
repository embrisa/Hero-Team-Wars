...
## JASS API lookup through the MCP

- The WC3 Map MCP exposes four global, read-only jassdoc tools: `jass_search`,
  `jass_lookup`, `jass_validate_call`, and `jass_validate_source`. They are
  available independently of project map allow-lists.
- Never invent JASS native/function names or signatures from memory. Search by
  concept when the API name is uncertain, look up the exact declaration and
  documentation, then validate individual calls or the complete source before
  staging a script change. Correct validation errors before retrying.
- The local API index is generated by
  `tools/wc3-map-mcp/scripts/sync-jassdoc.ps1` from the pinned `lep/jassdoc`
  commit. The generated `map-engine/data/jassdoc/jass-api.json` and upstream
  checkout are intentionally ignored; run bootstrap/sync when the dataset is
  missing and do not commit either one.
