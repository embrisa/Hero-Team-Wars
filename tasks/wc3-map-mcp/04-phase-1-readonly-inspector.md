...
- Runtime config: `tools/wc3-map-mcp/config/wc3-map-mcp.example.json` (`read_only`, exact six-tool allow list).
- Codex config: `.codex/config.toml` (`wc3_map`, STDIO, exact six-tool allow list).
- Build/test command: from `tools/wc3-map-mcp`, run `./scripts/build.ps1` and `./scripts/test.ps1`.
- Automated evidence: 13/13 .NET tests and 16/16 TypeScript/MCP tests passed. Coverage includes exact tool visibility, schema errors, text plus structured content, expected `isError`, clean stdout, worker timeout/crash/malformed/multiple-response mapping, stable repeated inspection, source preservation, traversal/absolute/sibling-root/directory/outside-root/symlink policy, validation hash, component status, and semantic/container diff separation.
- MCP Inspector CLI manually initialized the built STDIO server, listed exactly the six tools, and called project status, metadata inspection, supported `regions`, unsupported `variables`, and validation. The unsupported component returned `UNSUPPORTED_COMPONENT` with `isError: true`.
- `codex mcp list` and `codex mcp get wc3_map` recognized the project-scoped STDIO server and its exact six-tool allow list. A separate ephemeral, read-only `codex exec` session connected to `wc3_map`, called `wc3_project_status`, confirmed the exact six enabled tools, and confirmed mutation/build/launch/deletion are disabled. A desktop task that was already open before the config was added still requires restart/reload.
...
- Parsed read-only components: metadata, players, forces, regions, archive inventory/capabilities. Cameras, triggers, variables, scripts, object data, placed-object detail, terrain detail, and imports are explicit opaque/unsupported values rather than inferred data.
